...
class LoadingScene(Scene):
    def __init__(self, screen, sound_mixer, clock,loading_time=2):
        ...
        SC_WIDTH = self.screen.get_width()
        SC_HEIGHT = self.screen.get_height()
        self.clock = clock
        self.loading_time = loading_time
        
        self.text_surface, self.text_rect = self.build_text_surface(SC_WIDTH, SC_HEIGHT)

        FRAME_PER_SECOND = 24
        self.delay_per_frame = loading_time * FRAME_PER_SECOND // 8

        self.animation = self.build_loading_animation(SC_WIDTH, SC_HEIGHT)
    ...
    def build_loading_animation(self, SC_WIDTH, SC_HEIGHT):
        initial_pos = ((SC_WIDTH -1)//4, (SC_HEIGHT -1)//2)
        final_pos = ((SC_WIDTH -1)*3//4, (SC_HEIGHT -1)//2)
    ...
    def build_text_surface(self, SC_WIDTH, SC_HEIGHT):
        font = pygame.font.Font("fonts/PixeloidSansBold.ttf", 64)
        text = "Ahorcado"
        text_surface = font.render(text, True, (0, 0, 0))
        text_rect = text_surface.get_rect(center=(SC_WIDTH // 2, SC_HEIGHT // 4))
